...
@app.route("/predict the thal(Thalassemia)", methods=['GET','POST'])
def predict():
    form = PredictForm()
    if form.validate_on_submit():
        flash(form.data['age'],'success')
    return render_template('predict the thal(Thalassemia).html', title='predict the thal(Thalassemia)', form=form)

@app.route("/predict the heart disease", methods=['GET','POST'])
def predict_():
    form = PredictForm()
    if form.validate_on_submit():
        flash(form.data['age'],'success')
    return render_template('predict the heart disease.html', title='predict the heart disease', form=form)
